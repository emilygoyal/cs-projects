...
class State:
    ...
    def  generate_successors(self):
        """creates and returns a list of State objects for all successor states of the called State object. We discussed the concept of successor states in lecture
        """
        empty_list = []
        for x in MOVES:
            board_copy = self.board.copy()
            if board_copy.move_blank(x) == True:
                new = State(board_copy, self, x)
                empty_list += [new]
        return empty_list
